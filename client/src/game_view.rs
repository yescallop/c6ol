...
pub fn GameView(
    ...
    let heart = || {
        view! {
            <svg viewBox="0 0 128 128">
                ...
                <path
                    fill="#c33"
                    d="M30.65 11.2c17.2 0 25.74 18.49 28.5 25.98.39 1.07 1.88 1.1 2.33.06L64 31.35C60.45 20.01 50.69 8.97 34.03 8.97c-6.9 0-14.19 2.28-19.86 7.09 5.01-3.29 10.88-4.86 16.48-4.86zm63.34-2.23c-5.29 0-10.11 1.15-13.87 3.47 2.64-1.02 5.91-1.24 9.15-1.24 16.21 0 30.72 12.29 24.17 40.7-5.62 24.39-38.46 53.98-48.49 65.27-.64.72-.86 1.88-.86 1.88s51.39-37.21 58.38-67.08c6.98-29.86-10.53-43-28.48-43z"
                />
                <path
                    fill="#ff8a80"
                    d="M17.04 24.82c3.75-4.68 10.45-8.55 16.13-4.09 3.07 2.41 1.73 7.35-1.02 9.43-4 3.04-7.48 4.87-9.92 9.63-1.46 2.86-2.34 5.99-2.79 9.18-.18 1.26-1.83 1.57-2.45.46-4.22-7.48-5.42-17.78.05-24.61zm60.12 9.84c-1.76 0-3-1.7-2.36-3.34 1.19-3.02 2.73-5.94 4.58-8.54 2.74-3.84 7.95-6.08 11.25-3.75 3.38 2.38 2.94 7.14.57 9.44-5.09 4.93-11.51 6.19-14.04 6.19z"
                />
            </svg>
        }
    };
    ...
    let previous_move = move || {
        ...
        match mov {
            ...
            Move::Win(p, dir) => EitherOf4::B(win_rings(p, Some(dir), WIN_RING_COLOR)),
            Move::Pass | Move::Draw | Move::Resign(_) => {
                let text = match mov {
                    Move::Pass => "PASS",
                    Move::Draw => {
                        if DRAW_AS_HEART.load(Ordering::Relaxed) {
                            return EitherOf4::D(heart());
                        }
                        "DRAW"
                    }
                ...
                EitherOf4::C(centered_text(text, fill))
            }
        }
    };
